...
3. **Use `scripts/ad.ts` for everything.** Never call image providers inline — use the unified tool with `generate`, `render`, or `full` subcommands.
4. **Write a campaign.md brief**, not JSON. The agent writes Markdown, the script parses it.
5. **No text in AI-generated images.** Every prompt must explicitly say "no text, no words, no letters, no logos."
...
13. **Max 3 retries per ad.** After 3 failed attempts, grade F, flag in report, and move on.
14. **Output to `output/creatives/{campaign-name}/`.** One directory per campaign.
