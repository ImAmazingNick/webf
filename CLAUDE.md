...
- `knowledge/creatives/creative-workflow.md` — **Start here for ad creative builds.** Pipeline steps, fal.ai prompt patterns, compositing rules.
- `knowledge/creatives/creative-design-guide.md` — **Visual design bible.** Composition, color, typography, 5 layout patterns, prompt engineering, overlay rules, QA checklist, anti-patterns.
- `knowledge/creatives/platform-specs.md` — 4 ad sizes (square, portrait, landscape, story), platform rules, layout guides.
- `knowledge/creatives/ad-copy-guide.md` — Headline formulas, CTA patterns, character limits, tone rules for short-form ad copy.
- `.claude/agents/creative-generator.md` — Ad creative agent. Reads brief → writes campaign.md → runs `ad.ts` → outputs PNGs.
- `scripts/ad.ts` — Unified ad tool. Generates AI backgrounds (multi-provider), composites text + logo via Playwright, outputs @2x PNGs.
...
### Creative rules
1. **Read the creative workflow first** (`creative-workflow.md`) before generating any ad.
2. **Requires `FAL_KEY` environment variable.** All models (Flux, Grok Imagine, Nano Banana) run through fal.ai.
3. **Use `scripts/ad.ts` for everything.** Subcommands: `generate`, `render`, `full`, `refine`.
4. **Write a campaign.md brief**, not JSON. The agent writes Markdown, the script parses it.
5. **Use `- negative-prompt:` for exclusions** instead of appending "no text" to prompts. Standard: `text, words, letters, writing, characters, watermarks, logos`.
6. **Text is composited via Playwright**, not generated by AI. The `ad.ts render` subcommand handles text + logo overlay.
7. **Ad copy from messaging files when available.** Adapt for length, never fabricate claims or stats.
8. **Headlines: 3–7 words.** CTA: 2–4 words. Body (optional): 8–15 words.
9. **Brand colors in every visual.** Prompts must reference deep purple, violet, and/or mint.
10. **Lock seeds for campaign consistency.** Set `seed` in campaign.md config — same seed used for all sizes.
11. **Generate all 4 sizes** per variant automatically. The script handles square, portrait, landscape, story.
12. **Review every output against `creative-rubric.md`.** Vision-check each ad for text readability, AI artifacts, brand compliance, and layout.
13. **Use `--variant=N` for targeted iteration.** Never regenerate all variants when fixing one. Use `refine` for img2img when composition is 90% right.
14. **Set `fallback-models` in config** for resilience (e.g., `fal:flux-pro, fal:flux-schnell`). Retries default to 3 (production) or 10 (explore mode).
15. **Output to `output/creatives/{campaign-name}/`.** One directory per campaign. Images are cached — re-running with same prompt+seed+model skips API calls.
...
1. Read `knowledge/creatives/creative-workflow.md` (pipeline steps, prompt patterns, MD brief format)
2. Read `knowledge/creatives/creative-design-guide.md` (visual composition, prompt engineering)
3. Read `knowledge/creatives/platform-specs.md` (ad sizes, platform rules)
4. Read `knowledge/creatives/ad-copy-guide.md` (headline formulas, tone rules)
5. Read the relevant messaging file for content source
6. Read `knowledge/branding/improvado-agent.md` (brand rules)
7. Run `@creative-generator` with campaign name and messaging source
8. Output lands in `output/creatives/{campaign-name}/`
